...
router.post('/test-webhook', async (req, res) => {
  try {
    const { eventType = 'meeting.participant_joined', meetingId = 'test_meeting_123' } = req.body;
    ...
    const testEvent = {
      ...
      payload: {
        object: {
          ...
          end_time: eventType === 'meeting.ended' ? new Date().toISOString() : null,
          duration: 60,
          participant: eventType.includes('participant') ? {
            participant_uuid: `test_participant_${Date.now()}`,
            id: 'test_participant_123',
            user_name: 'Test Participant',
            email: 'participant@example.com',
            user_id: 'test_user_456',
            join_time: new Date(Date.now() - 300000).toISOString(), // 5 minutes ago
            leave_time: eventType === 'meeting.participant_left' ? new Date().toISOString() : null,
            client_type: 'Web'
          } : null
        }
      },
      meetingId: meetingId,
      meetingUuid: `test_uuid_${Date.now()}`,
      meetingTopic: 'Test Meeting for Webhook',
      participant: eventType.includes('participant') ? {
        participant_uuid: `test_participant_${Date.now()}`,
        user_name: 'Test Participant',
        email: 'participant@example.com'
